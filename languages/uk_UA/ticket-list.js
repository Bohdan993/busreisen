const translations = {
    "months": {
        0: "Січ.",
        1: "Лют.",
        2: "Берез.",
        3: "Квіт.",
        4: "Трав.",
        5: "Черв.",
        6: "Лип.",
        7: "Серп.",
        8: "Верес.",
        9: "Жовт.",
        10: "Листоп.",
        11: "Груд."
    },
    "fromText": "Звідки",
    "toText": "Куди",
    "priceTextFull": "Стара ціна",
    "priceText": "Ціна за пасажира",
    "buttonSelectText": "Обрати",
    "destinationText": "Туди",
    "returnText": "Назад",
    "onboardingText": "Місце посадки",
    "outboardingText": "Місце висадки",
    "departureTimeText": "Час відправлення",
    "arrivalTimeText": "Час прибуття",
    "additionalText": "час прибуття на наступну добу",
    "additionalText2": "час прибуття залежить від проходу кордону",
    "continueText": "Далі",
    "totalSumText": "Загальна сума",
    "takeTicketErrorText": "Оберіть квиток, щоб продовжити",
    "changeText": "Змінити",
    "passengerTexts": ["пасажир", "пасажири", "пасажирів"],
    "ticketsTitle": "Ваш рейс",
    "alternativeTicketsTitle": "Найближчі доступні рейси",
    "discountText": "Знижка",
    "discountCardNumberText": "Номер карти",
    "validationErrorText": "Заповніть це поле",
    "attentionText": "Зверніть увагу!",
    "termsOfUseSaleTicketText": {
        1: "Для акційних квитків персональні знижки пасажирів не діють.",
        2: "Акційні квитки обміну та поверненню не підлягають."
    },
    "termsOfUseOpenDateTicketText": {
        1: "Квитки з відкритою датою обміну та поверненню не підлягають."
    }
}



module.exports = translations;